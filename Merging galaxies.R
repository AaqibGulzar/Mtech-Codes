...
map <- "merging_galaxies"


img <- raster::raster("assets/Webb_explores_a_pair_of_merging_galaxies.tif")
...
shadow_depth <- min(mat, na.rm = TRUE) - 25000
...
mat %>%
  ...
  height_shade() %>%
  add_overlay(overlay = png::readPNG("assets/out.png")) |> 
...
if (!dir.exists(glue("plots/day_9/{map}"))) {
  dir.create(glue("plots/day_9/{map}"))
}
...
# GitHub difficult. 
outfile <- str_to_lower(glue("plots/day_9/{map}/{map}_img_z{res}.png"))
