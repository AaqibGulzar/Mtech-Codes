...
ggplot(at_site_sk, aes(x = seq_along(Tasker.skew), y = Tasker.skew)) +
  ...
  geom_point(aes(x = seq_along(Skew), y = Skew, color = "Skew"), size = 3) +
  geom_line(aes(x = seq_along(Skew), y = Skew, color = "Skew"), size = 1) +
  scale_x_continuous(breaks = seq_along(at_site_sk), labels = seq_along(at_site_sk)) +
  labs(x = "Value", y = "Value", color = "Variable")  +
  scale_color_manual(values = c("green", "blue")) +
  guides(color = guide_legend(title = "Variable")) + theme_void()

# plot using plotly
library(plotly)
p = plot_ly(
  at_site_sk,
  x = seq_along(at_site_sk$Tasker.skew),
  y = at_site_sk$Tasker.skew,
  mode = "markers+lines",
  type = "scatter",
  ...
  layout(title = "Connected Dot Plot",
    xaxis = list(title = "Station Number"),
    yaxis = list(title = "Skewness"),
    legend = list(title = "Variable"))

#plotly_IMAGE (p,"Skew Plot.png")


# Using Base R plot

plot(at_site_sk$Tasker.skew,type="o",col = "green",ylab = "Skewness")
points(at_site_sk$Skew,type="o",col = "blue",ylab  = "Skewness")
legend("bottomright", legend = c("Tasker.skew", "At Site Skew"),
...
gls = optim(par = x_gls, MLE_gls, method = "Nelder-Mead")
                        ...
                        # OR                  <|  These Give Almost Same Values |>
...
#to get the 50year flood quantiles for other regions.(this is as per bulletin 17-B)
...
Quantiles$Qest[1] + 1.96*sd_Q[1]/(rlv[1])^0.5       # 10268.94
Quantiles$Qest[1] - 1.96*sd_Q[1]/(rlv[1])^0.5       # 7850.441
