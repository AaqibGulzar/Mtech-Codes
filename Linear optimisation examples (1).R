...
bet = matrix(rep(1,15,nrow = 15))
idm = diag(1,nrow = 92,ncol = 92)
X = model.matrix(ols_reis)
MLE = function(x){matrix(c(
  0.5*log(det(x[1]*idm + var_mat),base=exp(1)) + 
    0.5*t((Reis_Et_Al_2020_Skew$skew - X%*%x[2])) %*% solve((x[1]*idm +var_mat)) %*%
    (Reis_Et_Al_2020_Skew$skew - X%*%x[2])))
}
optim(par= c(1,bet),MLE,method="Nelder-Mead")
