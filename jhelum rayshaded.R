...
t1 = create_texture("red","green","blue","yellow","white")
...
elevation_jhe <- elevatr::get_elev_raster(locations =sf_jhelum, z = 8, clip = "locations")

mat = raster_to_matrix(elevation_jhe)

bigmb = resize_matrix(mat, scale=2, method="cubic")

bigmb %>%
  sphere_shade(zscale=3, 
               texture = t1) %>% 
                 
                 
                 # create_texture(lightcolor =  "orange",shadowcolor = "brown",
                 #                        leftcolor = "red",rightcolor = "red",
                 #                        cornercolors = "white",centercolor = "white")) %>%
  add_shadow(ambient_shade(bigmb, maxsearch = 100, multicore = TRUE,zscale=1),0) %>%
  add_shadow(lamb_shade(bigmb),0.5) %>%
  plot_map()

water_palette = colorRampPalette(c("darkblue", "dodgerblue", "lightblue"))(200)
bathy_hs = height_shade(bigmb, texture = water_palette)
plot_map(bathy_hs)
#Set everything below 0m to water palette
bigmb %>%
  sphere_shade(zscale=10) %>%
  add_overlay(generate_altitude_overlay(bathy_hs, bigmb, 0, 0)) %>%
  add_shadow(ray_shade(bigmb,zscale=50),0.3) %>%
  plot_map()
#Add snow peaks by setting `lower = FALSE`
snow_palette = "white"
snow_hs = height_shade(bigmb, texture = snow_palette)
#Set the snow transition region from 500m to 1200m
bigmb %>%
  sphere_shade(zscale=10, texture = "desert") %>%
  add_overlay(generate_altitude_overlay(bathy_hs, bigmb, 0, 0)) %>%
  add_overlay(generate_altitude_overlay(snow_hs, bigmb, 500, 1200, lower=FALSE)) %>%
  add_shadow(ambient_shade(bigmb,zscale=50,maxsearch=100),0) %>%
...
base_map = mbay %>%
  height_shade() %>%
  add_overlay(generate_altitude_overlay(bathy_hs, bigmb, 0, 0)) %>%
  add_shadow(lamb_shade(bigmb,zscale=50),0.3)
#Plot a compass
base_map %>%
  add_overlay(generate_compass_overlay(heightmap = bigmb)) %>%
  plot_map()
#Change the position to be over the water
base_map %>%
  add_overlay(generate_compass_overlay(heightmap = bigmb, x = 0.15)) %>%
  plot_map()
#Change the text color for visibility
base_map %>%
  add_overlay(generate_compass_overlay(heightmap = bigmb, x = 0.15, text_color="white")) %>%
  plot_map()
